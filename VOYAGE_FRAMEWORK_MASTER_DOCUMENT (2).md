...
│  │  │  fallback)  │  │  │(4-tier) │ │(ChromaDB│ │  │  │(whitelist│ │ │  │
│  │  │             │  │  └─────────┘ └─────────┘ │  │  │+ audit)│  │ │  │
│  │  │  Append-only │  │  ┌─────────┐              │  │  └───────┘  │ │  │
│  │  │  Replayable  │  │  │  AST    │              │  │  ┌───────┐  │ │  │
...
├── pyproject.toml
├── README.md
├── RULES.md                    # ← Живой документ (auto-updated)
├── ADR/
...
│   ├── memory/
│   │   ├── __init__.py
│   │   ├── context_tiers.py    # 4-tier + tiktoken budgeting
│   │   ├── semantic.py         # VectorStore abstraction → ChromaDB
│   │   └── ast_manager.py      # Unified facade: ASTPython + ASTTypeScript
│   ├── specs/
│   │   ├── __init__.py
│   │   ├── adr.py              # ADR creation + storage
│   │   ├── plan.py             # Micro-phase planning
│   │   ├── task_generator.py   # Генерация TASK.md для Kimi Code
...
|-----------|-----------|
| **sandbox.py** | Блокировка опасных команд |
...
**Semantic Memory:** ChromaDB с коллекциями по типам. Fallback на SQLite keyword search.
...
**Механизм:**
1. Слушает `error_logged` events
2. Извлекает `error_type` и `pattern`
3. Ищет похожие ошибки в истории
...
│  - Deploy
└────────┬────────┘
         │ log: micro_phase_done
         ▼
┌─────────────────┐
│  Event Store    │
│  - Все события append-only
...
│     ├─ For each step:                                       │
│     │   ├─ Run tools through SecureExecutor                 │
│     │   ├─ Collect ToolResults                              │
│     │   └─ Log tool_executed events                         │
...
- `timestamp`, `agent_id`, `role`, `command`, `result`, `exit_code`, `blocked`, `reason`, `approval_id`

Можно replay: "что делал AI вчера в 15:00?"
...
```
        ...
        │   🟡 Средний    │     Role-aware filtering
        │   приоритет     │
        └────────┬────────┘
                 │
        ┌────────▼────────┐
        │   SEMANTIC      │  ← Похожие решения по смыслу
        │   (семантическая)│    Vector search
...
Human: "Почему отказались от UUID?"
    │
    ▼
SemanticMemory.query("Why UUID was rejected")
    │
    ▼
ChromaDB finds:
...
│    - Найти похожие ошибки в истории                         │
...
│    - Hash rule_text                                         │
│    - Проверить RULES.md (по hash или text search)           │
│    - Если exists → skip                                     │
│                                                             │
│  Step 4: Apply                                              │
│    - Atomic append в RULES.md                             │
│    - Log rule_added event                                   │
...
| Event Engine | 🟡 | `core/event_engine.py` |
...
| Tool Adapters | 🟡 | `tools/adapters/*.py` |
| Context Tiers | 🟡 | `memory/context_tiers.py` |
| Semantic Memory | 🟡 | `memory/semantic.py` |
| AST Manager | 🟡 | `memory/ast_manager.py` |
| Specs (ADR, Plan, Task, Tracker) | 🟡 | `specs/*.py` |
| Self-Improving Engine | 🟡 | `self_improving/engine.py` |

**AC:** 10 acceptance criteria (см. System Prompt)
...
- ✅ Core Models + Event Engine
- ✅ Security Sandbox + Approval
- ✅ Tool Engine + Adapters
- ✅ 4-tier Memory + Semantic
- ✅ AST Manager (Python + TS)
- ✅ Specs (ADR, Plan, Task, Tracker)
- ✅ Self-Improving Engine
...
- [ ] Создать папку `F:\WORK\skilltracer	oolsoyage_framework\`
- [ ] Создать подпапки (core, security, tools, memory, specs, agents, workflows, integrations, visualizer_api, tests)
- [ ] Скопировать `RULES.md`
- [ ] Скопировать 4 ADR в `ADR/`
- [ ] Создать `pyproject.toml`
- [ ] `pip install -e ".[dev]"`
- [ ] Запустить Kimi Agent с System Prompt
- [ ] Сгенерировать `core/models.py`
...
- [ ] Все 10 AC Phase 1 пройдены
- [ ] `mypy voyage_framework/` — clean
- [ ] `ruff check voyage_framework/` — clean
- [ ] `pytest tests/` — all passed
- [ ] `RULES.md` содержит ≥5 правил (включая auto-generated)
- [ ] EventEngine логирует события без ошибок
- [ ] Sandbox блокирует dangerous commands
- [ ] TaskGenerator создаёт валидный TASK.md
- [ ] SelfImprovingEngine добавляет правила из ошибок

### Чек-лист запуска Visualizer (Phase 3)

- [ ] Phase 2 завершена
- [ ] Visualizer API сервер запущен (`localhost:8000`)
- [ ] SSE `/events/stream` возвращает события
...
sim = SelfImprovingEngine(engine, rules_path=Path("RULES.md"))
...
| **v3.0** | Event Store (SQLite), AST (Python ast), Tools (no sandbox), State Machine (broken branching) | PostgreSQL+SQLite, tree-sitter multi-lang, Secure Sandbox, real branching |
| **v3.1** | Sandbox (buggy), Models (ULID), Agent Runtime (concept) | Fixed sandbox, full runtime, self-improving |
| **v4.0** | — | Всё выше + Spec-Driven + Visualizer API + Dual-Location |
