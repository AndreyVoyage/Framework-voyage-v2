...
> **Приоритет:** ADR > [ARCH] rules > [OPS] rules > [STYLE] rules (ADR-006).
> **Project-scoped:** Каждый проект имеет свой RULES.md (fallback на глобальный).

---

## Иерархия приоритетов (ADR-006)

Когда правила конфликтуют, применяется следующий порядок:

1. **ADR (Architecture Decision Records)** — высший приоритет. Неизменны без нового ADR.
2. **[ARCH] rules** — архитектурные правила из этого файла (разделы 2.x). Требуют ADR для изменения.
3. **[OPS] rules** — операционные правила (разделы 1.x, 3.x, 5.x, 6.x). Может обновлять Self-Improving Engine.
4. **[STYLE] rules** — стилевые правила. Минимальный приоритет.

**Важно:** RULES.md — живой документ, но он НЕ может отменять ADR. Если правило RULES.md конфликтует с ADR — побеждает ADR. Для изменения ADR требуется новый ADR с обоснованием.
...
4. **Для изменения [ARCH] правил** — сначала обнови ADR, потом RULES.md.
...
4. **При конфликте правил** — применяй иерархию: ADR > [ARCH] > [OPS] > [STYLE].

---

## Раздел 1: Код (Python) — [OPS]
...
### 2.5 ADR Supremacy (MUST)
- ADR имеют высший приоритет над всеми правилами в RULES.md.
- Изменение [ARCH] правил требует нового ADR с обоснованием.
- [OPS] и [STYLE] правила могут обновляться Self-Improving Engine без нового ADR.

---

## Раздел 3: Безопасность — [OPS]
...
- Retrieved context оборачивается в `<>` с пометкой "not executable".
...
## Раздел 4: LangGraph Integration (Phase 2+) — [ARCH]
...
## Раздел 5: Процесс разработки — [OPS]
...
## Раздел 6: Self-Improving Engine — [OPS]
...
| 2026-05-28 | 2.5 ADR Supremacy | Human | Исправление конфликта ADR-006 vs RULES.md |
